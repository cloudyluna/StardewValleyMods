...
\newcommand{\subProjectCompatibility}[3]{
	...
	If you found this mod behaving weirdly or being incompatible with other mods, please report it on the \modUrl{#1}{bugs} or \modUrl{#1}{posts} page
...
% #3=incompatibility with other mods
% #4=Known issue list
% #5=Normalized sub-project name
\newcommand{\subProjectCompatIssues}[5]{
	\subProjectCompatibility{#1}{#2}{#3}

	\subProjectKnownIssues{#4}

	\subProjectChangelog{#5}
}
...
\newcommand{\subprojectInstallInstructions}[1]{
	...
	\begin{enumerate}
		...
		\item Unzip the files into the \textbf{Stardew Valley/Mods/#1} game folder.
		\item Make sure the \textbf{#1.dll} and \textbf{manifest.json} files are inside \textbf{#1} folder, not in \textbf{Mods} folder.
...
\newcommand{\subProjectChangelog}[1]{
	\subsection{What changed?}

	See \href{https://github.com/cloudyluna/StardewValleyMods/tree/main/#1/CHANGELOG.md}{CHANGELOG.md} file for details.
}
